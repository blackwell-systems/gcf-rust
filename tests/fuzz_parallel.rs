...
fn gen_value(rng: &mut u64, depth: usize, max_depth: usize) -> Value {
    ...
    match rng_next(rng) % 6 {
        ...
        5 => {
            ...
            Value::Array((0..n).map(|_| gen_value(rng, depth + 1, max_depth)).collect())
        }
        _ => gen_scalar(rng),
    }
}
...
fn values_equal(a: &Value, b: &Value) -> bool {
    match (a, b) {
        ...
        (Value::Object(a), Value::Object(b)) => a.len() == b.len() && a.iter().all(|(k, v)| b.get(k).map_or(false, |bv| values_equal(v, bv))),
        _ => false,
    }
}

fn run_parallel(name: &str, iterations: usize, seed_offset: usize, gen: impl Fn(&mut u64) -> Value + Sync) {
